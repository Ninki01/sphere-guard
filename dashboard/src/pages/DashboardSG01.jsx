...
function DashboardSG01() {
  ...
  const sendDriveCommand = (mode) => {
    ...
    const commandRef = ref(rtdb, 'robots/sg01/control/movement');
  ...
  const handleSteeringRelease = () => {
    const clickTime = Date.now();
    const steeringRef = ref(rtdb, 'robots/sg01/control/steering_command');
  ...
  const startInspectionSession = async () => {
    ...
    try {
      ...
      await update(ref(rtdb, 'robots/sg01/system'), { activeSessionId: newSessionId });
  ...
  const stopInspectionSession = async () => {
    ...
    try {
      ...
      await update(ref(rtdb, 'robots/sg01/system'), { activeSessionId: null });
    ...
    <div className="dashboard-container">
      ...
      <header className="top-bar">
        <div style={{ display: 'flex', alignItems: 'center', gap: '14px' }}>
          ...
          <div>
            <h1 className="title">SG-01 Controller</h1>
        ...
        <main className="right-column">
          ...
          <div className="sensor-grid" style={{ gridTemplateColumns: 'repeat(4, 1fr)', gap: '15px', marginBottom: '24px' }}>
            ...
            <div className="light-card">
              ...
              <div className="card-value">{sensorData.batteryVoltage} <span className="unit">V</span></div>
            </div>
          ...
          <div className="dark-panel" style={{ marginBottom: '24px', padding: '15px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '15px' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '5px', fontSize: '12px', color: C.red }}>
                <span className="status-dot red"></span> OFFLINE
              </div>
            </div>
            <div style={{
              width: '100%', height: '350px', backgroundColor: 'rgba(255,255,255,0.4)',
              borderRadius: '8px', border: `1px solid ${C.border}`,
              display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', color: C.sage,
            }}>
